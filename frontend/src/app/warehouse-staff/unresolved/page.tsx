...
export default function UnresolvedIncidentsPage() {
  ...
  const fetchUnresolvedIncidents = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5169/api/WarehouseStaff/incidents/unresolved', {
        method: 'GET',
        headers: {
      ...
      console.error('未解決インシデントの取得エラー:', err);
      setError('未解決インシデントの取得に失敗しました。');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <Button onClick={fetchUnresolvedIncidents} variant="outline">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">未解決インシデント</h1>
              <p className="text-gray-600 mt-1">
                担当倉庫: <span className="font-semibold text-blue-600">{user?.warehouseName || '不明'}</span>
              </p>
            </div>
            <Button onClick={fetchUnresolvedIncidents} variant="outline" size="sm">
              <RefreshCw className="h-4 w-4 mr-2" />
              更新
            </Button>
          </div>
          ...
          <CardContent>
            {incidents.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">未解決のインシデントがありません</p>
              </div>
            ) : (
