...
export default function ClassifiedIncidentsPage() {
  ...
  // 分類済みインシデント一覧の取得
  const fetchClassifiedIncidents = async () => {
    try {
      setLoading(true);
      setError(null);

      const response = await fetch('http://localhost:5169/api/WarehouseStaff/incidents/classified', {
        method: 'GET',
        headers: {
      ...
      console.error('分類済みインシデントの取得エラー:', err);
      setError('分類済みインシデントの取得に失敗しました。');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <Button onClick={fetchClassifiedIncidents} variant="outline">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">分類済みインシデント</h1>
              <p className="text-gray-600 mt-1">
                担当倉庫: <span className="font-semibold text-blue-600">{user?.warehouseName || '不明'}</span>
              </p>
            </div>
            <Button onClick={fetchClassifiedIncidents} variant="outline" size="sm">
              <RefreshCw className="h-4 w-4 mr-2" />
              更新
            </Button>
          </div>
          ...
          <CardContent>
            {incidents.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">分類済みのインシデントがありません</p>
              </div>
            ) : (
