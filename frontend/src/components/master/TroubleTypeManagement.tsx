...
import { TroubleType, CreateTroubleTypeDto, UpdateTroubleTypeDto } from '@/lib/types';
import ColorPicker from '../common/ColorPicker';

export default function TroubleTypeManagement() {
  const [troubleTypes, setTroubleTypes] = useState<TroubleType[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreateForm, setShowCreateForm] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [formData, setFormData] = useState<CreateTroubleTypeDto>({
    name: '',
    description: '',
    color: '#3B82F6',
    sortOrder: 0
  ...
  const loadTroubleTypes = async () => {
    try {
      setLoading(true);
      const data = await apiClient.getTroubleTypes();
      setTroubleTypes(data);
    } catch (error) {
      console.error('トラブル種類の読み込みに失敗しました:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await apiClient.updateTroubleType(editingId, {
          ...formData,
          isActive: true
        });
        setEditingId(null);
      } else {
        await apiClient.createTroubleType(formData);
      }
      
      setFormData({ name: '', description: '', color: '#3B82F6', sortOrder: 0 });
      setShowCreateForm(false);
      await loadTroubleTypes();
    } catch (error: any) {
      if (error.errors) {
  ...
  const handleEdit = (troubleType: TroubleType) => {
    setEditingId(troubleType.id);
    setFormData({
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('このトラブル種類を削除しますか？')) return;
    
    try {
      await apiClient.deleteTroubleType(id);
      await loadTroubleTypes();
    } catch (error) {
  ...
  const handleToggleActive = async (id: number, currentStatus: boolean) => {
    try {
      const troubleType = troubleTypes.find(t => t.id === id);
      if (troubleType) {
        await apiClient.updateTroubleType(id, {
          name: troubleType.name,
          description: troubleType.description || '',
          color: troubleType.color,
          sortOrder: troubleType.sortOrder,
